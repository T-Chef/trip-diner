...
/**
 * ✅ 전국 모드: “골고루 인기”
 * - 각 지역은 이미 arrange=D(조회수)로 가져옴
 * - 지역별로 정렬된 리스트를 만든 뒤, 라운드로빈으로 교차해서 다양성 확보
 */
...
// ✅ 전국 모드
const NATIONAL_AREA_CODES = [1, 2, 3, 4, 5, 6, 7, 8, 31, 32, 33, 34, 35, 36, 37, 38, 39];
...
export async function getPlaces(reqQuery) {
  ...
  if (hasArea) {
    ...
    // ✅ 각 지역별로 가져오면서 “출처 areaCode”를 심어둠
    const chunks = await mapWithConcurrency(NATIONAL_AREA_CODES, 3, async (ac) => {
      try {
  ...
  if (useEnhance) {
    ...
    await mapWithConcurrency(targets, 3, async (p) => {
      if (p.image) return;
      try {
        const enhancedImg = await enhanceImage(p.title, p.latitude, p.longitude);
  ...
  if (useAI) {
    ...
    await mapWithConcurrency(targets, AI_CONCURRENCY, async (p) => {
      try {
        const aiText = await retry(() => generateDescription(p.title, p.address), {
          retries: 2,
          baseDelay: 250,
        });
