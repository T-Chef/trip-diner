      ...
      const places = (d.plan_item || [])
        ...
        .map((it) => {
          ...
          return {
            ...
            startTime,
            endTime,
            contentId: pl.content_id ?? pl.contentId ?? pl.external_id ?? null,
            contentTypeId: pl.content_type_id ?? pl.contentTypeId ?? null,
            areaCode: pl.area_code ?? pl.areaCode ?? memo?.areaCode ?? null,
          };
        });
...
export default function AIScheduleSummary() {
  ...
  const openPlaceDetail = (p) => {
    ...
    const type = p?.contentTypeId ?? p?.content_type_id ?? 12;

    const area = p?.areaCode ?? p?.area_code ?? 0;

    if (!contentId) {
      alert(
        "이 장소는 상세페이지로 이동할 ID(contentId)가 없어요. (DB/AI 저장 시 external_id 저장 필요)"
      );
  ...
  const downloadPDF = async () => {
    ...
    const daySections = Array.from(
      root.querySelectorAll(".summary-day-section")
    );
      ...
      <div className="summary-page">
        ...
        <div ref={pdfRef}>
          <header className="summary-hero">
            <h1 className="summary-title">
              {aiPlan.cityName || aiPlan.title}, {nights}박 {totalDays}일
              추천일정입니다.
            </h1>
        ...
        <footer className="summary-footer">
          ...
          <p className="summary-footer-text">
            마음에 드는 일정을 내 일정으로 담으면 언제든지 확인하고 편집할 수
            있어요.
          </p>
