...
export default function BoardDetail() {
  ...
  useEffect(() => {
    if (id) {
      ...
      if (user_id) {
        axios
          .get(`${API_BASE}/like/post/${id}/status?user_id=${user_id}`)
          .then((res) => {
  ...
  const toggleLike = async () => {
    if (!user_id)
      return Swal.fire("로그인 필요", "로그인 후 가능합니다.", "warning");
    try {
  ...
  const CommentItem = ({ comment, depth = 0 }) => (
    <div
      ...
      style={{ marginLeft: `${depth * 25}px`, width: "100%" }}
    >
        ...
        <img
          src={
            comment.user?.profile_img
              ? `http://localhost:4000${comment.user.profile_img}`
              : "/default_profile.png"
          }
      ...
      {comment.children.map((child) => (
        <CommentItem key={child.comment_id} comment={child} depth={depth + 1} />
      ))}
    </div>
  );
      ...
      <div className="board-detail board-wrapper">
        <div className="detail-header">
          <div
            ...
            onClick={() => navigate(`/board/list/${post.category}`)}
          >
            {post.category} 게시판 &gt;
          </div>
              ...
              <img
                src={
                  post.user?.profile_img
                    ? `http://localhost:4000${post.user.profile_img}`
                    : "/default_profile.png"
                }
        ...
        <div className="comment-input-area">
          {replyTo && (
            <div className="reply-notice">
              <span>
                <strong>@{replyTo.name}</strong> 님에게 답글 작성 중
              </span>
